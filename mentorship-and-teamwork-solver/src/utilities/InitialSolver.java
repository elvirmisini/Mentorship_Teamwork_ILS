...
public class InitialSolver {
    ...
    public static List<Assignment> solveMentorshipAndTeamwork(List<Project> projects, List<Contributor> contributors) {
        Map<UUID, List<String>> contributorIdWithSkillNamesMap = contributors.stream().collect(Collectors.toMap(Contributor::getId, contributor -> contributor.getSkills().stream().map(Skill::getName).collect(Collectors.toList())));
        Map<UUID, Map<String, Integer>> contributorIdAndSkillNameWithLevel = contributors.stream().collect(Collectors.toMap(Contributor::getId, contributor -> contributor.getSkills().stream().collect(Collectors.toMap(Skill::getName, Skill::getLevel))));
        ...
        for (Project project : projects) {
            ...
            Map<UUID, String> contributorIdAndSkillNameToIncrease = new HashMap<>();
            Map<UUID, String> contributorIdAndSkillNameToAdd = new HashMap<>();
                    ...
                    if (!addedSkill.contains(projectSkill.getId())) {
                        if (!assignedContributorIdsToProject.contains(contributor.getId())) {
                            if (contributorIdWithSkillNamesMap.get(contributor.getId()).contains(projectSkill.getName())) {
                                ...
                                else if (contributorSkillLevel + 1 == projectSkill.getLevel() && hasMentor(projectSkill.getLevel(), projectSkill.getName(), assignedContributorsToProject)) {
                                    addedSkill.add(projectSkill.getId());
                                    assignedContributorsToProject.put(assignedContributorsToProject.size() + 1, contributor);
                                    assignedContributorIdsToProject.add(contributor.getId());
                                    contributorIdAndSkillNameToIncrease.put(contributor.getId(), projectSkill.getName());
